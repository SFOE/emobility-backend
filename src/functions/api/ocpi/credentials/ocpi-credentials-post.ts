...
import {
  partySecretExists,
  savePartySecret,
} from '/opt/nodejs/aws/secrets-manager';
import {
  getPrimaryRole,
  validateCredentialsPayload,
} from '/opt/nodejs/utils/ocpi-utils';

export const handler = withVersionCheck((_, auth) =>
  assertIsBootstrap(auth, 'credentials/post'),
)(async (
  ...
  try {
    ...
    if (!bodyResult.success) {
      console.warn(
        `[OCPI][credentials/post] Rejected — invalid or missing request body from ${authContext.partnerId}`,
      );
    ...
    if (validationError) {
      console.warn(
        `[OCPI][credentials/post] Validation failed for ${authContext.partnerId}:`,
        validationError,
      );
    ...
    if (await partySecretExists(primaryRole)) {
      console.warn(
        `[OCPI][credentials/post] Rejected — already registered: ${partyRef}`,
      );
      return ErrorHandler.handleBadRequestError(
        2001,
        `${partyRef} is already registered.`,
        405,
      );
    }
    ...
    const secretRef = await savePartySecret(
      primaryRole,
      credentials.token,
      newToken,
    );
    ...
    // const bootstrapToken = extractToken(
    //   event.headers?.authorization || event.headers?.Authorization,
    // );
    // if (!bootstrapToken) {
    //   throw new Error(
    //     'Bootstrap token could not be extracted from authorization header',
    //   );
    // }
    // await invalidateBootstrapToken(bootstrapToken);

    console.info(
      `[OCPI][credentials/post] Registration successful: ${partyRef}`,
    );
    ...
    console.error(
      `[OCPI][credentials/post] Unexpected error for party ${authContext.partnerId}:`,
      err,
    );
