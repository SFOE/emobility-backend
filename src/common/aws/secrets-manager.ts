...
/**
 * Static IAM access keys for an S3 bucket in a foreign AWS account,
 * stored as a JSON secret (e.g. the swisstopo data.geo.admin.ch bucket).
 */
export interface S3AccessKeySecretValue {
  ACCESS_KEY_ID: string;
  SECRET_ACCESS_KEY: string;
}

/**
 * Loads static S3 access keys from AWS Secrets Manager.
 *
 * Unlike `getPartySecret`, a missing or incomplete secret is not tolerated: it
 * is a deployment error, so the error propagates and aborts the invocation.
 */
export const getS3AccessKeySecret = async (
  secretId: string,
): Promise<S3AccessKeySecretValue> => {
  const response = await secretsClient.send(
    new GetSecretValueCommand({
      SecretId: secretId,
    }),
  ...
  const secret = JSON.parse(response.SecretString) as S3AccessKeySecretValue;

  if (!secret.ACCESS_KEY_ID || !secret.SECRET_ACCESS_KEY) {
    throw new Error(
      `Secret ${secretId} is missing ACCESS_KEY_ID or SECRET_ACCESS_KEY.`,
    );
  }
...
export const partySecretExists = async (
  role: OCPICredentialRole,
): Promise<boolean> => {
  const secretName = `/emobility/ocpi/parties/${role.role}/${role.country_code}/${role.party_id}`;
  try {
