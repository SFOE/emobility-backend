...
export const Aws = {
  ...
  // swisstopo bucket (data.geo.admin.ch) that serves the published GeoJSON on geo.admin.ch
  get swisstopoBucketName(): string {
    return getRequiredLambdaEnv('SWISSTOPO_BUCKET_NAME');
  },
  get swisstopoGeoJsonKeyPrefix(): string {
    return getRequiredLambdaEnv('SWISSTOPO_GEOJSON_KEY_PREFIX');
  },
  get swisstopoCredentialsSecretName(): string {
    return getRequiredLambdaEnv('SWISSTOPO_CREDENTIALS_SECRET_NAME');
  },
  region: 'eu-central-1',
  swisstopoBucketRegion: 'eu-west-1',
  s3Config: {
    region: 'eu-central-1',
