...
const MOCK_S3_KEY =
  'connector/year=2025/month=01/day=01/country=CH/party=EMS/location_id=LOC001/evse_uid=EVSE001/connector_id=1/PATCH_20250101T000000000Z.json';
  ...
  describe('last_updated validation', () => {
    it('returns 400 with OCPI 2001 when last_updated is missing from the patch body', async () => {
      const result = await handler(
        buildConnectorPatchEvent({
          body: JSON.stringify({ max_amperage: 16 }),
        }),
      );
    ...
    it('returns 400 with OCPI 2001 when last_updated is an empty string', async () => {
      const result = await handler(
        buildConnectorPatchEvent({
          body: JSON.stringify({ last_updated: '' }),
        }),
      );
