import {
  overlayStatus,
  parseStatusItems,
} from '../../../../src/functions/geojson-emitter/overlay';
import type {
  GoldLocation,
  StatusItem,
...
describe('parseStatusItems', () => {
  ...
  it('parses multiple items into distinct keys', () => {
    ...
    expect(result['LOCATION#CH#TIA#LOC002#EVSE#EVSE002']!.status).toBe(
      'RESERVED',
    );
  });
});
...
describe('overlayStatus', () => {
  it('overlays the live status onto a matching EVSE', () => {
    ...
    const statusByKey = parseStatusItems([
      makeStatusItem({ status: 'CHARGING' }),
    ]);
