...
import {
  run,
  writeGeoJson,
} from '../../../../src/functions/geojson-emitter/geojson-emitter';
import type {
...
describe('writeGeoJson — one file per language', () => {
  ...
  it('publishes the identical FeatureCollection under all four language keys', async () => {
    ...
    expect(inputs.map((input) => input.Key)).toEqual([
      `${PREFIX}/ch.bfe.ladestellen-elektromobilitaet_de.json`,
      `${PREFIX}/ch.bfe.ladestellen-elektromobilitaet_fr.json`,
      `${PREFIX}/ch.bfe.ladestellen-elektromobilitaet_it.json`,
  ...
  it('propagates a failed upload', async () => {
    const send = jest.fn().mockRejectedValue(new Error('S3 PutObject failed'));
